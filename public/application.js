...
var config = {
  apiOrigins: {
    production: 'https://phonophonia.herokuapp.com/'
  }
};
...
var showTracksTemplate = __webpack_require__(19);
var showPlaylistsTemplate = __webpack_require__(36);
var showCurrentTemplate = __webpack_require__(37);
var tracksAPI = __webpack_require__(10);
var playlistsAPI = __webpack_require__(11);
...
var getTracks = function getTracks() {
  tracksAPI.getTracks().then(getTracksSuccess).catch(getTracksError);
};

var getPlaylists = function getPlaylists() {
  playlistsAPI.getPlaylists().then(getPlaylistsSuccess).catch(getPlaylistsError);
};

var getTracksSuccess = function getTracksSuccess(data) {
  $('.tracks-list').empty();
  var userTracks = data.tracks.filter(function (track) {
    return track.user_id === store.userData.id;
  });
  var nonUserTracks = data.tracks.filter(function (track) {
    return track.user_id !== store.userData.id;
  });
  if (userTracks === 0) {
  ...
  var showUserTracksHtml = showTracksTemplate({ tracks: userTracks });
  $('.tracks-list').append(showUserTracksHtml);
  var showNonUserTracksHtml = showTracksTemplate({ tracks: nonUserTracks });
  ...
  $('.delete-track').on('click', function (event) {
    ...
    tracksAPI.deleteTrack(id).then(deleteTrackSuccess).catch(deleteTrackFailure);
  });
  $('.edit-track').on('click', function (event) {
    event.preventDefault();
    var thisTrackSave = $(this).parent().parent().parent().siblings()[9];
    var thisTrackCancel = $(this).parent().parent().parent().siblings()[10];
    ...
    var trackID = $(this).parent().parent().parent().parent().attr('data-id');
    var trackTitle = $(this).parent().parent().parent().siblings()[0];
    trackTitle.contentEditable = true;
    var trackArtist = $(this).parent().parent().parent().siblings()[2];
    trackArtist.contentEditable = true;
    var trackURL = $(this).parent().parent().parent().siblings()[4];
  ...
  $('.cancel-to-playlist').on('click', function (event) {
    event.preventDefault();
    var trackTitle = $(this).parent().siblings()[0];
    trackTitle.contentEditable = false;
    var trackArtist = $(this).parent().siblings()[2];
    trackArtist.contentEditable = false;
    var trackURL = $(this).parent().siblings()[4];
  ...
  $('.add-to-playlist').on('click', function (event) {
    event.preventDefault();
    var thisTrackDropdown = $(this).parent().siblings()[6];
    var thisTrackCancel = $(this).parent().siblings()[9];
    $(thisTrackDropdown).children().removeClass('hidden');
    $(thisTrackCancel).children().removeClass('hidden');
    $('.add-to-playlist').addClass('hidden');
    $('.delete-track').addClass('hidden');
    $('.edit-track').addClass('hidden');
    playlistsAPI.getPlaylists().then(populatePlaylistList).catch(populatePlaylistError);
  });
};

var onSaveTrack = function onSaveTrack(trackID, trackTitle, trackArtist, trackURL) {
  var id = trackID;
  var newTitle = $(trackTitle).text();
  var newArtist = $(trackArtist).text();
  var newURL = $(trackURL).text();
  var data = {
    track: {
      title: newTitle,
      artist: newArtist,
  ...
  tracksAPI.editTrack(data, id).then(updateTrackSuccess).catch(updateTrackError);
};
var getTracksError = function getTracksError() {
  $('#message').html('<p>Something went wrong... did not retrieve tracks<p>');
  $('#message').show();
  $('#message').delay(2000).fadeOut('2000');
};

var getPlaylistsSuccess = function getPlaylistsSuccess(data) {
  $('.playlists-list').empty();
  var userPlaylists = data.playlists.filter(function (playlist) {
    return playlist.user_id === store.userData.id;
  });
  var nonUserPlaylists = data.playlists.filter(function (playlist) {
    return playlist.user_id !== store.userData.id;
  });
  if (userPlaylists === 0) {
  ...
  var showUserPlaylistsHtml = showPlaylistsTemplate({ playlists: userPlaylists });
  $('.playlists-list').append(showUserPlaylistsHtml);
  var showNonUserPlaylistsHtml = showPlaylistsTemplate({ playlists: nonUserPlaylists });
  $('.playlists-list').append(showNonUserPlaylistsHtml);
  $('.select-playlist').on('click', function (event) {
    event.preventDefault();
    var id = $(this).parent().parent().data('id');
    playlistsAPI.getAPlaylist(id).then(currentPlaylistSuccess).catch(currentPlaylistError);
  });
  ...
  $('.delete-playlist').on('click', function (event) {
    ...
    $('.confirm-changes-playlist').on('click', function (event) {
      ...
      // $('#message').show()
      // $('#message').delay(2000).fadeOut('2000'))
      playlistsAPI.deletePlaylist(id).then(deletePlaylistSuccess).catch(deletePlaylistFailure);
    });
  });
};

var getPlaylistsError = function getPlaylistsError() {
  $('#message').html('<p>Something went wrong... did not retrieve tracks<p>');
  $('#message').show();
  $('#message').delay(2000).fadeOut('2000');
};

var newPlaylistSuccess = function newPlaylistSuccess() {
...
var newPlaylistError = function newPlaylistError() {
  $('#message').html('<p>Something went wrong adding your playlist... try again?<p>');
  $('#message').show();
  $('#message').delay(2000).fadeOut('2000');
};

var currentPlaylistSuccess = function currentPlaylistSuccess(data) {
  $('.current-playlist').empty();
  var showCurrentHtml = showCurrentTemplate({ playlist: data.playlist });
  ...
  $('.remove-current-track').on('click', function (event) {
    event.preventDefault();
    var playlistID = $(this).parent().parent().parent().parent().attr('data-id');
    var playlistNameTarget = $(this).parent().parent().parent().siblings()[0];
    var playlistName = $(playlistNameTarget).text();
    var trackID = $(this).parent().parent().attr('data-id');
    var trackIDs = data.playlist.tracks;
    var newIDs = trackIDs.filter(function (obj) {
...
var updateCurrentPlaylist = function updateCurrentPlaylist(playlistID, playlistName, justIDs) {
  var id = playlistID;
  var newTrackIDs = justIDs;
  ...
  playlistsAPI.editPlaylist(data, id).then(updatePlaylistSuccess).catch(updatePlaylistFailure);
};

var currentPlaylistError = function currentPlaylistError() {
  $('#message').html('<p>Something went wrong getting playlist... try again?<p>');
  $('#message').show();
  $('#message').delay(2000).fadeOut('2000');
};

var populatePlaylistList = function populatePlaylistList(data) {
  $('.selectPlaylist').empty();
  $('.selectPlaylist').append($('<option value=0>Select your Mix</option>'));
  var userPlaylists = data.playlists.filter(function (playlists) {
    return playlists.user_id === store.userData.id;
  });
...
var selectedPlaylist = function selectedPlaylist(userPlaylists, value) {
  $('.current-playlist').empty();
  //   const pickPlaylist =
  //   { playlist: {
  ...
  $('.confirm-to-playlist').on('click', function (event) {
    ...
    playlistsAPI.addTrackToPlaylist(data).then(updatePlaylistSuccess).catch(updatePlaylistFailure);
  });
};

var populatePlaylistError = function populatePlaylistError() {
  $('#message').html('<p>Something went wrong getting your playlists... try again?<p>');
  $('#message').show();
  $('#message').delay(2000).fadeOut('2000');
};

var newSoundSuccess = function newSoundSuccess(data) {
  $('#message').html('<p>Sounds good to me!<p>');
  $('#message').show();
  $('#message').delay(2000).fadeOut('2000');
...
var newSoundError = function newSoundError() {
  $('#message').html('<p>Something went wrong adding your track... try again?<p>');
  $('#message').show();
  $('#message').delay(2000).fadeOut('2000');
};

var deleteTrackSuccess = function deleteTrackSuccess() {
...
var deleteTrackFailure = function deleteTrackFailure() {
  $('#message').html('<p>Something went wrong deleting your track... try again?<p>');
  $('#message').show();
  $('#message').delay(2000).fadeOut('2000');
};

var deletePlaylistSuccess = function deletePlaylistSuccess() {
  $('.current-playlist').empty();
  $('#message').html('<p>You will not hear them around here anymore...<p>');
  $('#message').show();
  $('#message').delay(2000).fadeOut('2000');
...
var deletePlaylistFailure = function deletePlaylistFailure() {
  $('#message').html('<p>Something went wrong deleting your track... try again?<p>');
  $('#message').show();
  $('#message').delay(2000).fadeOut('2000');
};

var updatePlaylistSuccess = function updatePlaylistSuccess() {
  $('#message').html('<p>Sounds good to me!<p>');
  $('#message').show();
  $('#message').delay(2000).fadeOut('2000');
  $('.selectPlaylist').empty();
...
var updatePlaylistFailure = function updatePlaylistFailure() {
  $('#message').html('<p>Something went wrong updating your playlist... try again?<p>');
  $('#message').show();
  $('#message').delay(2000).fadeOut('2000');
};

var updateTrackSuccess = function updateTrackSuccess() {
...
var updateTrackError = function updateTrackError() {
...
module.exports = {
  ...
  getPlaylistsSuccess: getPlaylistsSuccess,
  getPlaylistsError: getPlaylistsError,
  newPlaylistSuccess: newPlaylistSuccess,
  newPlaylistError: newPlaylistError,
  newSoundError: newSoundError,
  newSoundSuccess: newSoundSuccess
};
...
var newSound = function newSound(data) {
  var token = store.userData.token;
  return $.ajax({
...
var editTrack = function editTrack(data, id) {
  var token = store.userData.token;
  return $.ajax({
    url: config.apiOrigin + '/tracks/' + id,
    method: 'PATCH',
...
var newPlaylist = function newPlaylist(data) {
  var token = store.userData.token;
  return $.ajax({
...
var getAPlaylist = function getAPlaylist(id) {
  var order = $('.order-by').val();
  var token = store.userData.token;
  var data = {
    order: order
...
var editPlaylist = function editPlaylist(data, id) {
  var token = store.userData.token;
  return $.ajax({
    url: config.apiOrigin + '/playlists/' + id,
    method: 'PATCH',
...
var addTrackToPlaylist = function addTrackToPlaylist(data) {
  var token = store.userData.token;
  return $.ajax({
...
var trackEvents = __webpack_require__(38);
var playlistEvents = __webpack_require__(39);
...
var onSignIn = function onSignIn(event) {
  event.preventDefault();
  var data = getFormFields(this);
  api.signIn(data).done(ui.signInSuccess).catch(ui.signInFailure);
};

var onSignOut = function onSignOut(event) {
  event.preventDefault();
  var data = store.userData.id;
  api.signOut(data).done(ui.signOutSuccess).catch(ui.failure);
};

var onChangePassword = function onChangePassword(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.changePassword(data).done(ui.changePasswordSuccess).catch(ui.changePasswordFail);
};
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "      <div class=\"track\">\n      <ul data-id="
    ...
    + ">\n        <li><h2 class=\"track-title\">"
    + alias2(alias1((depth0 != null ? depth0.title : depth0), depth0))
    + "</h2></li>\n        <li><br><h4>Artist:</h4></li>\n        <li><p class=\"track-meta\">"
    + alias2(alias1((depth0 != null ? depth0.artist : depth0), depth0))
    + "</p></li>\n        <li><br><h4>Host:</h4></li>\n        <li><p>"
    + alias2(alias1((depth0 != null ? depth0.host_url : depth0), depth0))
    + "</p></li>\n        <li><br></li>\n        <li><button class= \"add-to-playlist btn btn-info btn-sm\">Add to Playlist</button></li>\n        <li><select class=\"selectPlaylist hidden\" name=\"selectPlaylist\"></select></li>\n        <li><button class= \"confirm-to-playlist hidden btn btn-info btn-sm\">Confirm</button></li>\n        <li><button class= \"save-changes hidden btn btn-info btn-sm\">Save Changes</button></li>\n        <li><button class= \"cancel-to-playlist hidden btn btn-info btn-sm\">Cancel</button></li>\n        <li class=\"track editable\">"
    ...
    return "\n          <ul>\n            <li><button class=\"btn btn-info btn-sm delete-track\">Delete Track</button></li>\n            <li><button class= \"edit-track btn btn-info btn-sm\">Edit Track</button></li>\n          </ul>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "<div class=\"row\">\n  <div class=\"col-sm-8 track-main\">\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.tracks : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "  </div>\n</div>\n";
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "      <ul data-id=\""
    ...
    + "\">\n        <li><h2 class=\"playlist-name\">"
    + alias2(alias1((depth0 != null ? depth0.name : depth0), depth0))
    + "</h2></li>\n        <li><button class= \"select-playlist btn btn-info btn-sm\">Select Playlist</button></li>\n        <li class=\"playlist editable\">"
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = blockParams[0][0]) != null ? stack1.editable : stack1),{"name":"if","hash":{},"fn":container.program(2, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "</li>\n      </ul>\n      <!-- </div> -->\n";
},"2":function(container,depth0,helpers,partials,data) {
    return "\n          <ul>\n            <li><button class=\"delete-playlist btn btn-info btn-sm\">Delete Playlist</button></li>\n            <li><button class= \"confirm-changes-playlist hidden btn btn-info btn-sm\">Confirm</button></li>\n          </ul>\n        ";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "<!-- <div class=\"row\">\n  <div class=\"col-sm-8 blog-main\"> -->\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.playlists : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "  <!-- </div>\n</div> -->\n";
},"useData":true,"useBlockParams":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "    <ul data-id="
    + alias2(alias1((depth0 != null ? depth0.id : depth0), depth0))
    + ">\n        <li><h2 class=\"current-playlist-name\">"
    + alias2(alias1((depth0 != null ? depth0.name : depth0), depth0))
    + "</h2></li>\n        <li class=\"current-playlist-tracks\">\n"
    + ((stack1 = helpers.each.call(alias3,(depth0 != null ? depth0.tracks : depth0),{"name":"each","hash":{},"fn":container.program(2, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "")
    + "</li>\n        <li class=\"playist editable\">"
    + ((stack1 = helpers["if"].call(alias3,(depth0 != null ? depth0.editable : depth0),{"name":"if","hash":{},"fn":container.program(4, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "")
    + "</li>\n    </ul>\n";
},"2":function(container,depth0,helpers,partials,data) {
  ...
  return "           <ul data-id="
    ...
    + "</li>\n            <li>Src: "
    + alias1(((helper = (helper = helpers.host_url || (depth0 != null ? depth0.host_url : depth0)) != null ? helper : alias3),(typeof helper === alias4 ? helper.call(alias2,{"name":"host_url","hash":{},"data":data}) : helper)))
    + "</li>\n            <li><button class=\"remove-current-track hidden btn btn-info btn-sm\">Remove Track</button></li>\n            <li><button class= \"save-changes-current hidden btn btn-info btn-sm\">Save Changes</button></li>\n          </ul>";
},"4":function(container,depth0,helpers,partials,data) {
    return "\n          <ul>\n            <li><button class= \"edit-current-playlist btn btn-info btn-sm\">Edit Playlist</button></li>\n            <li><button class= \"cancel-changes-current hidden btn btn-info btn-sm\">Cancel</button></li>\n          </ul>\n        ";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    var stack1;

  return "<!-- <div class=\"row\">\n  <div class=\"col-sm-8 playlist-main\"> -->\n"
...
var onNewSound = function onNewSound(event) {
  event.preventDefault();
  var data = getFormFields(this);

  api.newSound(data).then(ui.newSoundSuccess).catch(ui.newSoundError);
};

var onGetTracks = function onGetTracks() {
  event.preventDefault();
  api.getTracks().then(ui.getTracksSuccess).catch(ui.getTracksError);
};
...
var onNewPlaylist = function onNewPlaylist(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.newPlaylist(data).then(ui.newPlaylistSuccess).catch(ui.newPlaylistError);
};

var onGetPlaylists = function onGetPlaylists() {
  event.preventDefault();
  api.getPlaylists().then(ui.getTasksSuccess).catch(ui.getTasksError);
};

var onGetAPlaylist = function onGetAPlaylist() {
  event.preventDefault();
  api.getAPlaylist().then(ui.getAPlaylistSuccess).catch(ui.getAPlaylistError);
};
